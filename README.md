...
- [This
  Release](https://github.com/vertigis/studio-base-internal/releases/v15477024746-master)
  ( v15477024746-master )
...
> cd ~/deploy-studio
> curl -fsSL https://github.com/vertigis/studio-base-internal/archive/refs/tags/v15477024746-master.tar.gz | tar -xz
# If desired, use git:
> git clone --depth 1 --branch v15477024746-master https://github.com/vertigis/studio-base-internal ~/deploy-studio
> cd ~/deploy-studio
...
``` sh
# If session is direct (using xterm):
> ~/deploy-studio/activate.py
# If session is remote (using remote terminal):
> ssh -L localhost:7780:localhost:7780 user@host ~/deploy-studio/activate.py
```

## On Windows: Internal CA Certificates ( **OPTIONAL** )

Some environments may have internal Public Key Infrastructure (PKI) in
use. and some components of Studio may require proper alignment here. If
you need to integrate the the internal CA certificates of your
Enterprise into the container, follow these steps:
...
> ssh %REMOTE% cat ~/deploy-studio/extract-ca-certs.ps1 | powershell -Command -
# Copy the result back to the remote system
> scp -r certs-ca %REMOTE%:~/deploy-studio/
```
